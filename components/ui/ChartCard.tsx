    ...
    return (
        <div className="bg-white dark:bg-surface-dark p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col gap-6 h-full">
            <div className="flex items-start gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
                <div className={`size-12 rounded-2xl ${iconBgClass} flex items-center justify-center ${iconColorClass} flex-shrink-0`}>
                    <span className="material-symbols-outlined text-[24px]">{icon}</span>
                </div>
                <div className="flex-1 min-w-0 pt-1">
                    <h3 className="text-lg font-black text-slate-900 dark:text-white leading-tight break-words">{title}</h3>
                    <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mt-1 truncate">{subtitle}</p>
                </div>
            </div>
